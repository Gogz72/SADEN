...
def set_obstacle(data):
    global obstacles
    obstacles = np.array([data.pose.position.x, data.pose.position.y,1.0])
...
class Dot:
    def __init__(self, start, goal, gorientation, obstacles, k_att=0.5, k_rep=1, rep_radius = 1, max_iter=1000, threshold=0.1):
...
if __name__ == '__main__':
    ...
    obstacles = np.array([10000, 10000, 1])  # Convert to numpy array for easier calculations
