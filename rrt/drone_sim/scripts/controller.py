...
z_dot_pid = PID(2, 0, 0, setpoint=0)
#z_dot_pid = PID_Cont(2 , 0 , 0)
...
x_dot_pid = PID(8, 0, 0.6, setpoint=0)
y_dot_pid = PID(8, 0, 0.6, setpoint=0)
...
yaw_dot_pid = PID(1, 0.018, 0, setpoint=0)
yaw_dot_pid.output_limits = (-0.5, 0.5)       #maximum drone yaw rate
roll_pid = PID(0.1, 0.01, 0.05, setpoint=0)
pitch_pid = PID(0.1, 0.01, 0.05, setpoint=0)
 ...
 #   throttle_pid.setpoint= goal_point.position.z
  #  throttle_ = throttle_pid(current_state.position.z)
   ...
   # yaw_ = -yaw_pid(phi)
    ...
    # if yaw_dot_pid.setpoint > 0.5:
    #     yaw_dot_pid.setpoint = 0.5
    # elif yaw_dot_pid.setpoint < -0.5:
    #     yaw_dot_pid.setpoint = -0.5
    # if phi<0.05 and phi>-0.05:
    #     yaw_dot_pid.setpoint = 0
...
#    x_pid_output = x_pid(x_)    
#    y_pid_output = y_pid(y_)
    #print("delta = ",delta)
    ...
    #if x_>0.2:
    #    x_dot_pid.setpoint = 1
    # elif x_<-0.2:
    #     x_dot_pid.setpoint = -1
    # else:
    #     x_dot_pid.setpoint = 0
    # if y_>0.2:
    #     y_dot_pid.setpoint = 1
    # elif y_<-0.2:
    #     y_dot_pid.setpoint = -1
    # else:
    #     y_dot_pid.setpoint = 0

    roll_pid.setpoint =  y_dot_pid_output#-y_pid_output*0.2  #setpoint of the roll angle controller
    pitch_pid.setpoint = -x_dot_pid_output#+x_pid_output*0.2   #setpoint of the pitch angle controller
    ...
    control_commands=Quaternion(pitch_,roll_,throttle_+5.0,yaw_)
